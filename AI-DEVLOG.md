...
## [2026-04-07] 将休息状态切换改为系统空闲时间驱动

**改动类型**：重构

**涉及文件**：
- `apps/companion/src/main/main.ts` — 增加系统空闲时间轮询，并通过 IPC 向渲染层广播用户是否活跃
- `apps/companion/src/preload/preload.ts` — 暴露用户活跃状态查询与订阅接口
- `apps/companion/src/renderer/App.tsx` — 删除前端定时轮换空闲动作，改为根据用户活跃度在坐着和睡觉之间切换
- `README.md` — 更新 idle 状态说明
- `docs/architecture.md` — 更新状态机说明

**改动说明**：
按新的交互要求，不再让宠物在 idle 状态按固定时间轮换动作，而是读取系统键盘/鼠标空闲时间。用户有操作时宠物保持坐着；连续 1 分钟无操作时切换为睡觉；收到 started 或 progress 后仍优先进入 working 状态。

**注意事项**（可选）：
当前空闲检测使用主进程每 5 秒轮询一次系统空闲时间，因此从满 60 秒到切换睡觉，实际可能有最多约 5 秒的检测延迟。

---

## [2026-04-07] 调整工作态显示器为正向摆放

**改动类型**：重构

**涉及文件**：
- `apps/companion/src/renderer/pixelCat.ts` — 将猫咪和龙虾工作态的显示器倾斜角调整为 0

**改动说明**：
按最新视觉要求，去掉工作态显示器的斜放角度，改为正向摆放，保持现有构图不变，仅让屏幕观感更端正。

---

## [2026-04-07] 继续下移宠物头顶气泡

**改动类型**：重构

**涉及文件**：
- `apps/companion/src/renderer/styles.css` — 将头顶气泡继续下移 8px，贴近猫头但保留呼吸空间

**改动说明**：
在上一轮气泡位置微调基础上，继续将气泡整体下移一个更明显的幅度，使气泡与宠物本体的视觉关系更紧凑，减少悬空感。

---

## [2026-04-07] 微调气泡位置与猫咪工作态显示器构图

**改动类型**：重构

**涉及文件**：
- `apps/companion/src/renderer/styles.css` — 将头顶气泡整体下移一点，减少离窗口顶边过近的观感
- `apps/companion/src/renderer/pixelCat.ts` — 重排猫咪 working 状态的猫和显示器坐标，放大显示器并让左右两侧都能露出

**改动说明**：
根据新的视觉要求，将头顶气泡适度下移，避免位置过高；同时调整猫咪工作态构图，把猫放到更接近画面中心的位置，并将显示器放大到明显宽于猫身，让屏幕左右两边都能被看见，提升“在电脑前工作”的识别度。

---

## [2026-04-07] 修复宠物头顶气泡被窗口裁剪的问题

**改动类型**：Bug修复

**涉及文件**：
- `apps/companion/src/main/main.ts` — 提高宠物窗口高度，为头顶气泡预留显示空间
- `apps/companion/src/renderer/App.tsx` — 将气泡层抽成独立覆盖层，避免继续内联定位
- `apps/companion/src/renderer/styles.css` — 提高 dock 可视区域高度，并为气泡与昵称补充显示层级

**改动说明**：
气泡逻辑本身仍在工作，但宠物窗口高度与 canvas 高度相同，导致头顶气泡超出窗口上边界后被 Electron 直接裁掉。本次通过增大窗口和渲染区域高度，并明确气泡覆盖层层级，恢复头顶气泡显示。

**注意事项**（可选）：
后续如果继续放大宠物本体、调整底部停靠高度，需同时检查头顶气泡和昵称的可见空间，避免再次被窗口边界裁切。

---

## [2026-04-07] 调整全局 Copilot 指令为运行期自动启停

**改动类型**：重构

**涉及文件**：
- `apps/companion/src/main/main.ts` — 启动时无条件写入全局 instructions，退出前主动删除全局 instructions 文件
- `apps/companion/src/renderer/App.tsx` — 移除可切换的 Copilot 全局监听开关，改为展示运行期自动启停说明
- `apps/companion/src/renderer/styles.css` — 为设置页新增全局监听状态标签样式
- `README.md` — 更新全局 instructions 的自动写入与退出清理说明
- `docs/architecture.md` — 更新全局 instructions 生命周期说明

**改动说明**：
将 Companion 对 `~/.copilot/instructions/agentpet.instructions.md` 的处理从“按设置开关持久控制”调整为“应用运行期间自动启用，应用退出时自动清理”。这样可以保证 AgentPet 运行时 Copilot 一定能拿到全局指令，同时避免 Companion 关闭后全局指令残留继续影响后续会话。

**关联上下文**（可选）：
- 关联上次改动：[2026-04-07] 移除 VS Code 用户数据中的 instructions 副本同步
- 关联上次改动：[2026-04-07] 修复全局 Copilot 指令导致的 AgentPet MCP 不触发问题
